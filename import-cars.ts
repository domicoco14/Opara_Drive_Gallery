...
function parseCarDetails(folderName: string) {
  let make = 'Unknown'
  let model = 'Unknown'
  let year = 2020
  let price_ngn = 0
...
async function run() {
  ...
  const carsDir = path.join(process.cwd(), 'Ephraim Autos Available Cars')
  ...
  for (const folder of folders) {
    ...
    if (uploadedUrls.length > 0) {
      ...
      const { error: dbError } = await supabase.from('cars').insert({
        ...
        price_usd: null, // Let's leave USD empty or calculate it
